...
const hasSubmissionToday = (attempt, todayStart, todayEnd) => {
  const history = Array.isArray(attempt.submissionHistory)
...
const getAttemptScore = (attempt) => {
  ...
  return totalQuestions > 0
    ? ((attempt.correctCount ?? 0) / totalQuestions) * 10
    : 0;
};
    ...
    const suspiciousAttempts = attempts
      .filter(
        (attempt) =>
          ...
          (attempt.suspiciousExitCount ?? 0) >= 3
      )
      .slice(0, 30)
      .map((attempt) => {
        ...
        return {
          ...
          displayName:
            owner?.displayName ?? "Không rõ",
          username: owner?.username ?? "",
          userCode: owner?.userCode ?? "",
          examTitle: attempt.examTitle,
          subject: attempt.subject,
          suspiciousExitCount:
            attempt.suspiciousExitCount ?? 0,
...
export const getAdminStudentManagement =
  async (_req, res) => {
    try {
      ...
      const scoresToday = attemptsToday.map(
        (attempt) =>
      ...
      const studentRealtimeRows = users
        .slice(0, 80)
        .map((user) => {
          const userAttempts =
            attemptsByUser.get(
