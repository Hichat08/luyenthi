...
export const getAdminStudentManagement = async (_req, res) => {
  try {
    ...
    // Khi chưa có event "start exam", chỉ ước lượng "đang thi":
    // online và có bài nộp trong 2 giờ gần đây.
    const currentlyTakingExamCount = users.filter((user) => {
      if (!user.lastActiveAt || now - new Date(user.lastActiveAt).getTime() > ONLINE_WINDOW_MS) {
        return false;
      }
      const latestAttempt = (attemptsByUser.get(`${user._id}`) ?? [])[0];
      if (!latestAttempt?.submittedAt) return false;
      return now - new Date(latestAttempt.submittedAt).getTime() <= 2 * 60 * 60 * 1000;
    }).length;
    ...
    const studentRealtimeRows = users.slice(0, 80).map((user) => {
      ...
      if (
        ...
        now - new Date(lastAttempt.submittedAt).getTime() <= 2 * 60 * 60 * 1000
      ) {
        status = "taking_exam";
      }
