...
export default function AdminStudentManagementPage() {
 ...
 main
  ...
  const quickStats = useMemo(
    () => [
      ...
      { label: "Thí sinh online", value: data?.stats.onlineCandidates ?? 0, icon: Users },
      {
        label: "Đang làm bài",
        value: data?.stats.currentlyTakingExamCount ?? 0,
        icon: Activity,
      },
      { label: "Đã nộp hôm nay", value: data?.stats.submittedTodayCount ?? 0, icon: CheckCircle2 },
      { label: "Điểm trung bình", value: (data?.stats.averageScore ?? 0).toFixed(2), icon: Award },
      { label: "Điểm cao nhất", value: (data?.stats.highestScore ?? 0).toFixed(2), icon: Trophy },
      { label: "Tỉ lệ hoàn thành", value: `${data?.stats.completionRate ?? 0}%`, icon: Clock3 },
      { label: "Cảnh báo hôm nay", value: data?.stats.fraudAlertsTodayCount ?? 0, icon: ShieldAlert },
 ...
 main
  ...
  const getStatusMeta = (status: string, warnings: number) => {
    ...
    if (status === "submitted") return { label: "Đã nộp", tone: "text-amber-600" };
    if (status === "taking_exam") return { label: "Đang thi", tone: "text-emerald-600" };
    if (status === "online") return { label: "Online", tone: "text-sky-600" };
    return { label: "Offline", tone: "text-slate-500" };
    ...
    >
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        {quickStats.map(({ label, value, icon: Icon }) => (
          <article key={label} className="rounded-2xl border border-border/70 bg-card p-4 shadow-sm">
 ...
 main
      ...
      <section className="grid gap-4 xl:grid-cols-[1.4fr_1fr]">
        <article className="rounded-2xl border border-border/70 bg-card p-4">
          ...
          <h2 className="text-base font-black">Danh sách thí sinh realtime</h2>
          <p className="mt-1 text-xs text-muted-foreground">
            Ưu tiên hiển thị học viên mới nộp bài ở đầu danh sách.
          </p>
            ...
            <table className="w-full min-w-[620px] text-sm">
              <thead>
                <tr className="border-b border-border/70 text-left text-muted-foreground">
                  ...
                  <th className="py-2">Họ tên</th><th>Trạng thái</th><th>Thời gian</th><th>Điểm</th><th>Cảnh báo</th>
                </tr>
              </thead>
              <tbody>
                {studentRows.map((row) => {
                  const statusMeta = getStatusMeta(row.status, row.warningCount);
                  const submittedTimeLabel = row.lastSubmittedAt
                    ? new Date(row.lastSubmittedAt).toLocaleString("vi-VN", {
                        timeZone: "Asia/Ho_Chi_Minh",
                        hour12: false,
                      })
                    : "-";
                  return (
                    <tr key={row.userId} className="border-b border-border/50 last:border-b-0">
                      <td className="py-3 font-semibold">{row.displayName}</td>
                      <td className={statusMeta.tone}>{statusMeta.label}</td>
                      <td>{submittedTimeLabel}</td>
            ...
            {latestSubmissions.map((item) => (
              ...
              >
                <p className="flex items-start gap-2">
                  ...
                  <span>
                    [THÔNG BÁO MỚI] {item.displayName} vừa nộp bài - {item.subject || "Chưa rõ môn"} -{" "}
                    {item.score.toFixed(2)} điểm - {Math.round(item.timeSpentSeconds / 60)} phút
 ...
 main
      ...
      <section className="rounded-2xl border border-dashed border-primary/35 bg-primary/5 p-4 text-sm text-muted-foreground">
        <p className="flex items-center gap-2 font-semibold text-foreground"><Eye className="size-4 text-primary" />Gợi ý triển khai realtime</p>
        <p className="mt-2">Dùng Socket.IO để cập nhật nộp bài/cảnh báo không cần F5, lưu log hành vi (tab switch, copy/paste, IP, thiết bị) và bổ sung phân tích AI sau khi có dữ liệu thật.</p>
      </section>
    </AdminShell>
  );
}
